...
# Optional: Clean up the init container after startup
docker rm oai-init
```

**Note:** The `oai-init` container runs once to set up volume permissions and SSL certificates, then exits. It can be safely removed with `docker rm oai-init` after the system starts.

**SSL Certificates:**
- On first run, self-signed SSL certificates are automatically generated by the `cert-generator` init container and stored in `certs/`
- Certificates persist across container rebuilds (mapped to host directory)
- To use custom certificates, place `server.crt` and `server.key` in `certs/` before starting (must be readable by UID 101)
- For production, replace with trusted certificates from a certificate authority
- The nginx container runs as non-root user (UID 101) on unprivileged port 8443, mapped to host port 443
...
| **Rootless Containers** | All containers run as non-root users: API/workers (UID 1000), PostgreSQL (UID 999), Nginx (UID 101) |
